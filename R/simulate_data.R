simulateData <- function(.info_frame, .skewness, .kurtosis){
  if(is.null(.skewness) && is.null(.kurtosis)){
    ...
    stop("Please provide a list of skewness and kurtosis values for all of the indicators. Normally distributed indicators should have
    ...
    for(i in 1: length(.skewness)){
      ...
      coefficients[k,] <- Fleishman(.skewness = as.matrix(.skewness)[k,], .kurtosis = as.matrix(.kurtosis)[k,])
    }

    # Generate a matrix for the correlations of the standard normal variables
    cor_dependent <- matrix(0, nrow = nrow(.info_frame), ncol = ncol(.info_frame), dimnames = dimnames(.info_frame))
    for(i in 1:(nrow(.info_frame)-1)){
      for(j in (i+1):ncol(.info_frame)){
        cor_dependent[i,j] <- ValeMaurelli(.cor = .info_frame[i,j], .coef <- rbind(coefficients[i,], coefficients[j,]))
      }
    }
  ...
  Fleishman <- function(.skewness, .kurtosis){
    coef <- function(.x, .mom) {
      c(F1 = (.x[1]*.x[1] + 6*.x[1]*.x[3] + 2*.x[2]*.x[2] + 15*.x[3]*.x[3] - 1),
        F2 = (2*.x[2]*(.x[1]*.x[1] + 24*.x[1]*.x[3] + 105*.x[3]*.x[3] + 2) - as.numeric(.mom[1])),
        F3 = (24*(.x[1]*.x[3] + .x[2]*.x[2]*(1 + .x[1]*.x[1]+ 28*.x[1]*.x[3]) +
                    .x[3]*.x[3]*(12 + 48*.x[1]*.x[3] + 141*.x[2]*.x[2] + 225*.x[3]*.x[3])) - as.numeric(.mom[2])))
    }

    .mom <- rbind(.skewness, .kurtosis)
    res <- multiroot(coef, start = c(1,0,0), .mom = .mom, maxiter = 100, rtol = 1e-15)
...
# Calculate the correlation of the standard normal variables with the Vale Maurrelli approach
  ValeMaurelli <- function(.cor, .coef){
    corr <- function(.x, .cor, .coef){
      F1 = .x*(.coef[1,"b"]*.coef[2,"b"] + 3*.coef[1,"b"]*.coef[2,"d"] + 3*.coef[1,"d"]*.coef[2,"b"] +
                 9*.coef[1,"d"]*.coef[2,"d"]) + .x*.x*2*.coef[1,"c"]*.coef[2,"c"] + .x*.x*.x*6*.coef[1,"d"]*.coef[2,"d"] - .cor
    }
    res <- multiroot(corr, start = 0.5, .coef = .coef, .cor = .cor, maxiter = 100, rtol = 1e-5)
    return(res$root)
  }
